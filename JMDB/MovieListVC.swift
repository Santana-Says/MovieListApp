...
class MovieListVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "MovieDetail" {
            if let vc = segue.destinationViewController as? MovieDetailVC, cell = sender as? MovieCell, indexPath = tableView.indexPathForCell(cell)?.row {
                        vc.selectedMovie = movies[indexPath]
            }
        }
    }
}
